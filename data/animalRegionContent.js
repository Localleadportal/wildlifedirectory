...
const animalRegionContent = {
  ...
  'bat-removal': {
    ...
    Southeast: 'Big brown bats and little brown bats are the species most likely to form colonies inside homes across much of the Southeast, with free-tailed bats more common in structures along the Gulf Coast and lower coastal plain. All bats are protected, so the standard solution is humane exclusion rather than extermination. State wildlife agencies regulate or recommend against exclusion during the spring and summer maternity season, when flightless pups can be trapped inside; exact dates and whether the restriction is a rule or a recommendation vary by state, so exclusion timing should follow the guidance of the local state wildlife agency.',
  ...
  'bird-removal': {
    ...
    Southeast: 'The Southeast\'s warm climate and abundant food sources support high bird densities year-round. Non-native European starlings and house sparrows — neither protected by law — are the most common nuisance birds, nesting in dryer and bathroom vents, eaves, and signage cavities. Rock pigeons cluster on larger and commercial structures, and woodpeckers drum and excavate on wood-sided homes. Chimney swifts nest in chimneys and are fully protected under the federal Migratory Bird Treaty Act, so chimney work must be timed around their nesting season.',
