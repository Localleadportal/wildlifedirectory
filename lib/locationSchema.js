...
// Flat URL scheme: county is no longer a path segment, so the city trail is
// Home › State › City (› Service). The legacy 'county' / 'county-animal' cases
// are kept for the now-dead county templates but no longer reachable.
function buildCrumbs(pageType, f) {
  ...
  // City crumb links to the hub only when a citySlug is supplied. When the hub is
  // noindex (removed → 404), the caller passes no citySlug, so the crumb renders
  // as plain text (no `url`) and the JSON-LD omits its `item` — never a 404 link.
  const city   = f.citySlug
    ? { name: f.cityName, url: `/${f.stateSlug}/${f.citySlug}/` }
    : { name: f.cityName };
  switch (pageType) {
    ...
    case 'city':            return [home, state, { name: f.cityName }];
    case 'city-animal':     return [home, state, city, { name: f.serviceName }];
